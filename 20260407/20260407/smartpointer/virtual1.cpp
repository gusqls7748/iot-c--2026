/*
	객체 포인터의 다형성으로 기초 클래스 타입의 포인터로 파생 클래스의 객체를 가리키면 기초 클래스의 맴버에만 접근된다.
	이때 오버라이딩된 파생 클래스의 맴버 함수를 사용하기 위해서는 기초 클래스의 맴버 함수를 가상 함수로 만들면 된다.
