...
- auto를 쓰면 자동적으로 맞는 타입으로 바꿔준다.
- push_back은 뒤에꺼를 먼저 지운다

[소스](./20260403/exeprj01/exeprj/templater.cpp)
[소스](./20260403/exeprj01/exeprj/tempObj.cpp)
[소스](./20260403/exeprj01/exeprj/tempobj2.cpp)
[소스](./20260403/exeprj01/exeprj/tempobj3.cpp)

2. Vector
- 장점
    - 동적 크기 조절: 데이터가 몇 개 들어올지 몰라도 걱정 없습니다.
    - 연속된 메모리: 배열처럼 데이터가 메모리에 나란히 붙어 있어 접근 속도가 매우 빠릅니다.
    - 메모리 관리 자동화: 소멸자에서 알아서 메모리를 치워주므로, 개발자가 직접 delete를 할 필요가 없습니다. (객체 지향의 장점!)
    - 양한 함수 제공: 맨 뒤에 넣기(push_back), 크기 확인(size), 비우기(clear) 등 편리한 기능을 바로 쓸 수 있습니다.

[소스](./20260403/exeprj01/exeprj/vector.cpp)
[소스](./20260403/exeprj01/exeprj/vector2.cpp)
...
#### Vector 코드 분석 및 특징
- 작성하신 코드는 vector를 생성하고, 데이터를 삽입한 뒤, 총 4가지의 서로 다른 방법으로 데이터를 출력하는 과정을 담고 있습니다.

주요 특징 및 장점
- 안전한 원소 접근 (at): v[i] 방식보다 v.at(i)를 사용하면 범위를 벗어났을 때 예외를 발생시켜 주므로 더 안전하게 코딩할 수 있습니다.
- 유연한 데이터 타입 (size_type): 시스템의 메모리 크기에 따라 유연하게 대응하는 표준 부호 없는 정수 타입을 사용하여 안정성을 높였습니다.
- 반복자(Iterator) 활용: 포인터와 유사한 개념인 iterator를 사용하여 컨테이너의 시작(begin)부터 끝(end)까지 순회하며 데이터에 직접 접근합니다.
- 범위 기반 for문 (Range-based for): C++11부터 도입된 가장 간결하고 현대적인 순회 방식입니다

[소스](./20260406/vector03/vectorstudy/vector03.cpp)

### Iterator (반복자)

- 개념

    - 포인터의 일반화: 컨테이너(Vector, List 등)의 원소를 가리키고 가리키는 지점을 이동할 수 있게 해주는 포인터와 유사한 객체입니다.

    - Template 기반 다형성: 어떤 컨테이너라도 begin()과 end()만 있다면 동일한 인터페이스(접근 방식)로 데이터를 순회할 수 있게 변신합니다.
...
for (auto riter = v.rbegin(); riter != v.rend(); riter++) {
    std::cout << *riter << " "; // 50 40 30 20 10
}

# 주의: 역방향 반복자에서도 ++ 연산자를 사용합니다. 이는 논리적으로 **'다음 역방향 원소'**로 이동한다는 뜻이기 때문에 매우 직관적입니다.
```

[소스](./20260406/vector03/vectorstudy/iterator.cpp)


### Deque
- 양방향 큐: `앞(Front)과 뒤(Back) 양쪽에서 데이터를 넣고 뺄 수 있는 자료구조`입니다.
- 분할 메모리 구조: Vector처럼 하나의 연속된 덩어리가 아니라, 여러 개의 메모리 블록을 할당받아 관리합니다.
- 중간 삽입/삭제 (insert, erase): 반복자(iterator)를 이용해 원하는 위치에 데이터를 넣거나 뺄 수 있습니다.
- 상수 반복자 (const_iterator): 가리키는 원소의 값을 변경할 수 없는 반복자입니다. 읽기 전용으로 안전하게 데이터를 순회할 때 사용합니다.

- 장점
    - 양방향 끝단 최적화: push_back뿐만 아니라 push_front 작업도 매우 빠릅니다. (Vector는 앞에서 넣으려면 데이터를 다 뒤로 밀어야 해서 느립니다.)
    - 동적 크기 조절: 데이터가 늘어날 때 전체를 복사해서 이사할 필요 없이, 새로운 메모리 블록만 추가하면 됩니다.
    - 인덱스 접근 가능: 큐(Queue) 형태임에도 불구하고 배열처럼 []나 at()으로 중간 데이터에 바로 접근할 수 있습니다.
    - 코드 중복 방지
    - 타입 안정성

- [소스](./20260406/vector03/vectorstudy/deque.cpp)
- [소스](./20260406/vector03/vectorstudy/deque02.cpp)
